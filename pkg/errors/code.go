...
var (
	unknownCoder defaultCoder = defaultCoder{1, "An internal server error occurred"}
)
...
func Register(coder Coder) {
	if coder.Code() == 0 {
		panic("code `0` is reserved by `beyondinfo.com/baselib/go/base_package.git/errors` as unknownCode error code")
	}
...
func MustRegister(coder Coder) {
	if coder.Code() == 0 {
		panic("code '0' is reserved by 'beyondinfo.com/baselib/go/base_package.git/errors' as ErrUnknown error code")
	}
